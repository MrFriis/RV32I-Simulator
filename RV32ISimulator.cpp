/*
* @file RV32ISimulator.cpp
* @author Martin Friis
* Implementation for the RV32ISimulator class
*/
...
//RV32ISimulator Constructor
RV32ISimulator::RV32ISimulator(){
  pc_ = 0;
  length_=0;
  memory_ = NULL;
  capacity_ = 0;
...
bool RV32ISimulator::readFromFile(const char * filepath){
  FILE *fp;
  unsigned int * buffer;
  long int fileSize;
...
bool RV32ISimulator::writeToFile(const char * filepath){
...
bool RV32ISimulator::hasNext(){
  return ((pc_ < length_) & !ecall_);
}

void RV32ISimulator::printRegisters(){
...
void RV32ISimulator::printProgram(){
  std::cout << "Program" << '\n' << std::endl;
  for(int i=0; i<length_; i++){
    int instr;
    load(instr, 4*i, 4);
...
*/
bool RV32ISimulator::save(int w, unsigned int idx, unsigned int bytes){
...
bool RV32ISimulator::load(int &w, unsigned int sp, unsigned int bytes, bool u){
  w = 0;
  for(unsigned int i=0; i<bytes; i++){
...
void RV32ISimulator::printAsHex(unsigned int instr){
  int res = 0;
  std::cout << "0x";
  for(int i = 0; i<8; i++){
    res = (instr >> (4*(7-i))) & 0xf;
